...
class WorkoutStorageService {
  ...
  Future<bool> saveWorkout(WorkoutModel workout) async {
    try {
      ...
      // Error handled with return value
      return false;
    }
  }
  ...
  Future<List<WorkoutModel>> getAllWorkouts() async {
    try {
      ...
      final List<String> workoutStrings =
          prefs.getStringList(_workoutsKey) ?? [];

      // Convert to WorkoutModel objects
      final workouts = workoutStrings
          .map((workoutString) {
            try {
  ...
  Future<bool> deleteWorkout(String workoutId) async {
    try {
      ...
      if (updatedWorkoutStrings.length != workoutStrings.length) {
        final success =
            await prefs.setStringList(_workoutsKey, updatedWorkoutStrings);
        return success;
      }
